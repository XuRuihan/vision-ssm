...
class SSM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.qkv = nn.Conv2d(dim, self.attention_dim * 3, 1, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Conv2d(self.attention_dim, dim, 1, bias=proj_bias)
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x):
        ...
        if self.fused_attn:
            ...
            # This is an LTI version of SSM
            hf = self.attn_drop(
                torch.triu(q[:, 0].permute(0, 3, 2, 1) @ k[:, 0].permute(0, 3, 1, 2))
            ) @ v[:, 0].permute(0, 3, 2, 1)
            hb = self.attn_drop(
                torch.tril(q[:, 1].permute(0, 3, 2, 1) @ k[:, 1].permute(0, 3, 1, 2))
            ) @ v[:, 1].permute(0, 3, 2, 1)
            wf = self.attn_drop(
                torch.triu(q[:, 2].permute(0, 2, 3, 1) @ k[:, 2].permute(0, 2, 1, 3))
            ) @ v[:, 2].permute(0, 2, 3, 1)
            wb = self.attn_drop(
                torch.tril(q[:, 3].permute(0, 2, 3, 1) @ k[:, 3].permute(0, 2, 1, 3))
            ) @ v[:, 3].permute(0, 2, 3, 1)
            x = (
                torch.cat(
                    [
...
@register_model
def ssm_s18(pretrained=False, **kwargs) -> MetaFormer:
    model_kwargs = dict(
        depths=[3, 3, 9, 3],
        dims=[64, 128, 320, 512],
...
@register_model
def ssm_m36(pretrained=False, **kwargs) -> MetaFormer:
    model_kwargs = dict(
        depths=[3, 12, 18, 3],
        dims=[96, 192, 384, 576],
        token_mixers=[SepConv, SepConv, SSM, SSM],
